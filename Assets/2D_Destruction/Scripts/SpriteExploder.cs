...
public static class SpriteExploder {
    ...
    private static GameObject generateTriangularPiece(GameObject source, List<Vector2> tri, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat, Rigidbody2D rigidbodyParams)
    {
        ...
        if (rigidbodyParams != null)
        {
            rigidbody.mass = rigidbodyParams.mass;
            Debug.Log("MASS: " + rigidbodyParams.mass);
            rigidbody.angularDrag = rigidbodyParams.angularDrag;
            rigidbody.drag = rigidbodyParams.drag;
            rigidbody.gravityScale = rigidbodyParams.gravityScale;
        }
    ...
    private static GameObject generateVoronoiPiece(GameObject source, List<Vector2> region, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat, Rigidbody2D rigidbodyParams)
    {
        //Create Game Object and set transform settings properly
        GameObject piece = new GameObject(source.name + " piece");
        piece.transform.position = source.transform.position;
